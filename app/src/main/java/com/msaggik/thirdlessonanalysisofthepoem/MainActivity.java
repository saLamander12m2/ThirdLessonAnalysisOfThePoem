...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView print = findViewById(R.id.result_text);

        // конвертирование строки в массив и его сортировка
        poemArray = stringToArray(poem.toLowerCase());
...
//        poem.replace("-", " ");
//        poem.toLowerCase();
//        String[] masText = poem.split(" ");
//        Arrays.sort(masText);
//
//
//        for (int i = 0; i < masText.length; i++) {
//           print.append((i == masText.length - 1) ? masText[i] : masText[i] + ",\n");
//        }
        List<String> resultList = sortText(poemArray);

        for (int i = 0; i < resultList.size(); i++) {
            print.append(resultList.get(i) + "\n");
        }
    ...
    private List<String> sortText(String[] text) {
        List<String> alphabeticallyText = Arrays.asList(text);
        Collections.sort(alphabeticallyText);

        return alphabeticallyText;
    }
}